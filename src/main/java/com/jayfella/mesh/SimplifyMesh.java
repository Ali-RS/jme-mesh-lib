 ...
 *
 * Simplifies a mesh by removing vertices with angles lower than a given threshold.
 * The threshold increases with each iteration until the desired target has been met.
 * The aggression increases the rate at which the angle increases each iteration.
 *
 * The algorithm ignores edges, making it very suitable for terrain decimation.
 *
 */
public class SimplifyMesh {

    private static class SymetricMatrix {
    ...
    private float aggression = 6;
    private int maxIterations = 1000;
    private boolean complexNormals = true;
    ...
    public SimplifyMesh(Mesh mesh) {
        this.inMesh = mesh;
        targetCount = mesh.getTriangleCount() / 2;
    }
    ...
    /**
     * Sets the amount of triangles to attempt to reduce the mesh to.
     * @param targetCount the amount of triangles to attempt to reduce the mesh to.
     */
    ...
    /**
     * Sets the amount of triangles to attempt to reduce the mesh to.
     * @param targetPercent the amount of triangles to attempt to reduce the mesh to.
     */
    public void setTargetPercent(float targetPercent) {
        targetCount = (int) (inMesh.getTriangleCount() * targetPercent);
    }
    ...
    /**
     * sharpness to increase the threshold. 5..8 are good numbers. more iterations yield higher quality.
     * Minimum 4 and maximum 20 are recommended.
     *
     * @param aggression the aggressiveness of the reduction procedure.
     */
    ...
    /**
     * The maximum amount of times the algorithm will iterate the mesh to achieve the desired target.
    ...
    public boolean isComplexNormals() {
        return complexNormals;
    }

    public void setComplexNormals(boolean complexNormals) {
        this.complexNormals = complexNormals;
    }
    ...
    public Mesh simplify() {

        // init
        if (inMesh == null) {
            throw new RuntimeException("No mesh specified to simplify.");
        }
        ...
        */
        ...
        for (int iteration = 0; iteration < maxIterations; iteration++) {
            ...
            */
            ...
            if (triangle_count - deleted_triangles <= targetCount) {
                break;
            }
            ...
            //
            final double threshold = 0.000000001d * Math.pow(iteration + 3d, aggression);
    ...
    private Mesh createSimplifiedMesh(boolean complexNormals) {
        ...
        // positions
        FloatBuffer pb = BufferUtils.createFloatBuffer(vertices.size() * 3);

        for (Vertex vert : vertices) {
            pb.put(vert.p.x).put(vert.p.y).put(vert.p.z);
        }
        ...
        for (Triangle t : triangles) {
            ib.put(t.v[0]).put(t.v[1]).put(t.v[2]);
        }

        // normals
        Vector3f[] normArray = complexNormals ? normalizeMesh(triangles, vertices) : new Vector3f[vertices.size()];
